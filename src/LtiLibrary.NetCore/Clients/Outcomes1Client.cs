            ...
            string sourcedId, SignatureMethod signatureMethod = SignatureMethod.HmacSha1)
        {
            try
            {
                ...
                try
                {
                    ...
                    msg.SetupHeaders();
                    msg.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue(LtiConstants.ImsxOutcomeMediaType));

                    await SecuredClient.SignRequest(
                        client, msg, HttpMethod.Post, serviceUrl, xmlContent, consumerKey, consumerSecret, signatureMethod)
            ...
            string lisResultSourcedId, double? score, string text = null, string url = null, string ltiLaunchUrl = null, SignatureMethod signatureMethod = SignatureMethod.HmacSha1)
        {
            try
            {
                ...
                try
                {
                    ...
                    await SecuredClient.SignRequest(client, msg, HttpMethod.Post, serviceUrl, xmlContent, consumerKey, consumerSecret, signatureMethod)
                        .ConfigureAwait(false);
