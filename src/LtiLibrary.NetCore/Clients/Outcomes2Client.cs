...
using LtiLibrary.NetCore.PollyRetryPolicies;
...
namespace LtiLibrary.NetCore.Clients
{
    ...
    public static class Outcomes2Client
    {
            ...
            string consumerSecret, SignatureMethod signatureMethod)
        {
            try
            {
                var msg = new HttpRequestMessage(HttpMethod.Delete, serviceUrl);
                msg.SetupHeaders();

                await SecuredClient.SignRequest(client, msg, HttpMethod.Delete, serviceUrl, new StringContent(string.Empty), consumerKey, consumerSecret, signatureMethod);
                ...
                try
                {
                    ...
                    using (var stream = new MemoryStream())
                    {
                        ...
                        var RetryPolicy = HttpClientRetrayPolicy.RetryPolicy;
            ...
            EventHandler<Newtonsoft.Json.Serialization.ErrorEventArgs> deserializationErrorHandler) where T : class
        {
            try
            {
                var msg = new HttpRequestMessage(HttpMethod.Get, serviceUrl);
                msg.SetupHeaders();
                msg.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue(contentType));

                await SecuredClient.SignRequest(client, msg, HttpMethod.Get, serviceUrl, new StringContent(string.Empty), consumerKey, consumerSecret, signatureMethod);
            ...
            EventHandler<Newtonsoft.Json.Serialization.ErrorEventArgs> deserializationErrorHandler) where T : class
        {
            try
            {
                ...
                var msg = new HttpRequestMessage(HttpMethod.Post, serviceUrl)
                {
                    Content = httpContent
                };
                msg.SetupHeaders();

                await SecuredClient.SignRequest(client, msg, HttpMethod.Post, serviceUrl, httpContent, consumerKey, consumerSecret, signatureMethod);
            ...
            string consumerSecret, T content, string contentType, SignatureMethod signatureMethod)
        {
            try
            {
                ...
                msg.SetupHeaders();
                await SecuredClient.SignRequest(client, msg, HttpMethod.Put, serviceUrl, httpContent, consumerKey, consumerSecret, signatureMethod);
